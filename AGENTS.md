...
**CRITICAL - Reprint URL Rewriting Rigor**: URL rewriting must preserve structured-data ownership boundaries. Do not use `str_replace`, `preg_replace`, delimiter trimming, punctuation stripping, or format-specific guessing to rewrite URLs. Known structured formats must be parsed by their owning parser first (for example SQL tokens, PHP serialization, JSON, WordPress block markup, HTML attributes/text, and base64 payload layers). Only the resulting leaf text, or an unknown value intentionally treated as plain text, may be delegated to `URLInTextProcessor`. `URLInTextProcessor` owns plain-text URL span detection; callers must not repair or reinterpret its spans as Markdown, CSS, CSV, TOML, or other container syntax. If typed-format correctness is required, add or use a typed parser for that format before URL rewriting. Performance work must keep these invariants and should rely on native scanners, structured parser APIs, bounded caches, and measured hot-path reductions rather than byte-level URL replacement.
