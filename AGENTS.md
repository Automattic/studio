...
**IMPORTANT - Gate App-Specific Agent Tools**: The Studio Code agent tools (`apps/cli/ai/tools/`) run in several contexts: forked by the Desktop app or the `studio ui` server (a Studio UI is attached), standalone terminal `studio code` sessions, and the MCP server. A tool or prompt instruction that only makes sense with a UI attached (e.g. `refresh_browser` reloading the preview pane, `studio_present` widgets) or with a specific bridge (e.g. `share_screenshot` needs the Telegram remote session) **leaks into the other contexts unless explicitly gated** — the model gets told to call a tool that does nothing there. When adding an app- or context-specific tool or behavior, gate BOTH sides on the right flag in `resolveStudioToolDefinitions` (`apps/cli/ai/tools/index.ts`) — `emitChatArtifacts` = UI attached (derived from `process.send`), `remoteSession` = `STUDIO_REMOTE_SESSION=1` — AND its mentions in the system prompt (`buildLocalIntro` in `apps/cli/ai/system-prompt.ts`, which receives the same flags). Add tests for both branches (present when flagged, absent by default); the MCP server calls `resolveStudioToolDefinitions()` with no options, so correct gating covers it automatically.
