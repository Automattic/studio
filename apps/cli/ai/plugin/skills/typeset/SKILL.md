---
name: typeset
description: Improve typography — font hierarchy, sizing, readability, and consistency. Use when text feels generic, inconsistent, or poorly structured.
user-invokable: true
...
In WordPress, fonts can be set via:
- `theme.json` → `settings.typography.fontFamilies` (for block themes)
- `functions.php` → `wp_enqueue_style()` for Google Fonts or `@font-face` in `style.css`
...
Build a clear type scale in `theme.json` → `settings.typography.fontSizes` or CSS custom properties:
- **5 sizes cover most needs**: caption, secondary, body, subheading, heading
- **Use a consistent ratio** between levels (1.25, 1.333, or 1.5)
- **Combine dimensions**: Size + weight + color + space for strong hierarchy — don't rely on size alone
- **WordPress content pages**: Use fluid sizing via `clamp(min, preferred, max)` for headings. Keep body text fixed.
...
- Use semantic token names in `theme.json` (`--wp--preset--font-size--body`), not arbitrary values
