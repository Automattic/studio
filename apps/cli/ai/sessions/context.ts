...
export function resolveResumeSessionContext(
	...
	fallbackModel: AiModelId = DEFAULT_MODEL
	...
	// model auto-switches to `fallbackModel` (the user's preferred default)
	// so resumed sessions never pin a model we no longer offer.
	context.model = resolveSessionModel( resumeSession.entries, fallbackModel );
