...
- **The look image**: before the look options, one `generate_images` call with a single image, the site's first-screen scene in `landscape`, at `<site>/wp-content/uploads/studio-generated/look.jpg`, with the call-wide `imageGrade` left out and a neutral, versatile grade in `pageContext` ("natural light, true color, moderate contrast"). With `present_design_options`, pass `background: true` in the same turn as `pick_design`: the image generates while you write the drafts, and `present_design_options` waits for it. Every look's board shows this same photo under that look's `imagery` treatment from its `DESIGN.md` draft, so the user compares looks rather than photo content. A direction that rejects photography (its Imagery line says none, or type-only) passes no image: its board shows a pattern.
- **The site's set**: once the look is picked, one `generate_images` call with 3–4 images in that look, with its Imagery treatment (the direction's Imagery line, tuned to the brand, as `DESIGN.md`'s Imagery section states it) as the call-wide `imageGrade` and its `style` if it is not photographic: the first-screen scene again plus distinct supporting subjects (a detail, a place or a person, a product), each in the aspect ratio of the slot it is most likely to fill, at `<site>/wp-content/uploads/studio-generated/<name>.jpg`. With `present_design_options`, pass `background: true` in the same turn as the layout's `pick_design`, before writing `DESIGN.md`: the set generates while you write `DESIGN.md` and the sneak peeks, and `present_design_options` waits for it and reports every image. The layout sneak peeks draw on this set, and the build starts from it: place these files first, per the workflow above, and generate more only for slots they cannot fill, with the same `imageGrade`. Keep `look.jpg` for a look the treatment reproduced; delete it otherwise.
- **A failed or unavailable image** is not a blocker: the board shows a pattern, and a sneak-peek slot a solid color shape — `present_design_options` does this itself for an image that failed in the background, and its report says which; handle it as below before the build.
