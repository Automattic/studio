...
Products should ideally have a real, relevant image, and a storefront with product images looks far more complete than one with placeholder thumbnails. **Do not pass remote image URLs via `--images` with `src`.** WooCommerce derives the upload filename from the URL's basename, and the extension-less CDN URLs most image hosts return (e.g. `https://images.unsplash.com/photo-1589924691995-400dc9ecc119?w=600`) are rejected with `Invalid image: Sorry, you are not allowed to upload this file type`. Instead, put each image in the media library and reference its attachment ID: when `generate_images` is available, generate the product images with `mediaLibrary: true` (see the `imagery` skill), which reports each attachment ID; otherwise download each image to the site with a real extension and import it. The WooCommerce CLI requires a `--user`:
