...
- **Content post types** (public, browsable collections — menu items, team members, projects, services, events): `'public' => true`, `'has_archive' => true`, `'show_in_rest' => true`, `'menu_icon'` a sensible `dashicons-*`, and `'supports' => array( 'title', 'editor', 'custom-fields', 'thumbnail' )`. **Do NOT pass a `'rewrite'` argument with a custom `'slug'`, and do NOT set `'has_archive'` to a string slug** — let the archive and single URLs default to the theme-prefixed post-type key (e.g. `/<prefix>_menu_item/`). A CPT archive or single URL must NEVER share a base with a content page: if any page in the manifest has slug `X`, the CPT must not register `has_archive => 'X'` nor `rewrite => array( 'slug' => 'X' )` — a same-named CPT archive shadows the page (WordPress routes `/X/` to the archive instead of the page). The PRIMARY way users reach a collection is a PAGE carrying a Query Loop against the CPT (seeded separately); the per-CPT archive template is a secondary, prefixed default. Keep archive + REST visibility on so those page Query Loops resolve.
...
Prefer post meta over custom taxonomies for content facets. A "tier", "category", "type", or "status" facet on a content type (sponsor tiers, project categories, cuisines) is best modeled as a `register_post_meta` atom surfaced through a `core/post-meta` block binding — NOT a custom taxonomy. Query Loops in this generator filter by `postType` only; they do not filter by a custom taxonomy. Register a `register_taxonomy` (on `init`, `'show_in_rest' => true`, attached to its post type, and `'rewrite' => false` so its term archives cannot collide with a page) ONLY when the manifest explicitly declares a taxonomy for that type. Never register a taxonomy no manifest entry calls for, and never reference in markup a taxonomy you did not register.
