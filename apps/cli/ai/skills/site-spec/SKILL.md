...
1. Call `pick_design` with `options: 4` and up to two `chosen` pairs; it adds random pairs to make four distinct options for the user. Call it with `options: 1` and no `chosen` pairs — a single random draw — when a pick cannot happen: `AskUserQuestion` is unavailable, the user asked to be surprised or to skip the questions, or the brief names both the concept and the direction.
