...
Load the `visual-design` skill first — the catalogs, the DESIGN.md format, and the sneak-peek rules live there — and follow its "Concept and Direction" runbook. When `generate_images` is available, load the `imagery` skill with it. The design is settled in two steps, the look and then the layout, each with one `pick_design` call, repeated only when the user asks for other options:
...
1. **The look**: `catalog: "directions"`, asking "Which look should I build?". Skip this step when the active-site line names a design system and the user did not ask for a new look.
   - If `present_design_options` is available: when `generate_images` is available too, start the look image with `background: true`, in the same turn as `pick_design`. Pass each option's `DESIGN.md` draft (see the `visual-design` skill) as its `preview`, with that image and a one-line description of the feel.
   - Otherwise use `AskUserQuestion`, each option with a one-line description of the look.
   - When `generate_images` is available, generate the site's image set in the picked look (see the `imagery` skill) so the layout previews and the build have it; with `present_design_options`, pass `background: true` in the same turn as the layout's `pick_design`, before writing `DESIGN.md`. Write the picked look to `DESIGN.md` at the site root: the draft's front matter exactly as the user saw it, then every section.
