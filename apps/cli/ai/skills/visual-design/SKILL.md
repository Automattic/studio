...
5. Call `pick_design` once with every open side (`layoutCandidates` and `directionCandidates`). When `AskUserQuestion` is available and the user has not asked to be surprised or to skip the questions, pass `options: 4`: the tool draws four distinct concept-and-direction pairs and the user picks one (see "Letting the user pick" below). Otherwise the tool draws one pair. Build what was drawn or picked — do not call the tool again to get a different result, and do not substitute another entry. For a redesign that keeps its layout or its look, pass only the side that is open.
...
When `pick_design` returned several options, let the user pick before writing the Site Spec. Do not describe the options in prose first — the question does that.

- **In the Studio app** (`present_design_options` is available): first, when `generate_images` is available, load the `imagery` skill and follow its "Design options" section — one call, at most one hero image per option, each graded by its own direction. Then write one standalone HTML sneak peek per option showing the first screen only — the header or masthead and the hero as the concept shapes them, and the top of the next section — in the direction's Palette and Type (inline CSS; a Google Fonts `<link>` with a fallback stack is fine), with the site's real name, plausible copy, that option's generated image referenced by absolute path, and solid color shapes for any other image slot. Under ~120 lines each, no scripts. It is a taste of the look, not a page to reuse: do not port it into the theme. Call `present_design_options` once with the question ("Which look should I build?") and one option per pair in the order drawn: label `<Concept> × <Direction>`, a one-line description of the feel, and the HTML.
- **In the terminal** (`AskUserQuestion` only): call `AskUserQuestion` once with the same question and one option per pair in the order drawn: label `<Concept> × <Direction>` and a one-line description of how the first screen would look. No previews are rendered.

Build the picked pair. If the user typed their own answer, read it as a preference ("2 but darker") and apply it to the closest option.
