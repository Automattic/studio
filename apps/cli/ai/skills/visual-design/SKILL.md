...
- **Content width**: Set `settings.layout.contentSize` and `wideSize` in `theme.json` for the site type instead of keeping the scaffold defaults: 700–860px content for editorial reading columns, 960–1200px content for marketing, landing, and business pages, with `wideSize` 1200–1400px above it. Put columns, card grids, and galleries at `"align":"wide"` (see the `block-content` skill) so they use the wide width.
