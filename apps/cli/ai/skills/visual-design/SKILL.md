...
5. When choosing for the user to pick from, choose four directions that suit this site and differ from one another, from a safe fit to bold ones you could execute well; for the layout, choose two that suit the picked look, one safe and one bold, and the code adds two at random. Give each a one-line reason, and do not describe or defend them in prose: the user decides.
...
- **A draft**, for a design option, is the front matter and the Overview. It is rendered as a design board — swatches, type, buttons on the palette's surfaces, the site name, the option's image — so the user judges exactly the tokens. The picked draft becomes the file: tokens unchanged, every section written.
