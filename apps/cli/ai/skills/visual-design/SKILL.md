...
- **Style ownership**: Before implementation, make a quick ownership plan for the major visual decisions. Put global tokens/defaults in `theme.json`, page-specific editable styling in block attributes, repeated composition in patterns/templates, unsupported visual treatments in CSS, and reusable behavior in custom blocks or the Interactivity API. Load `theme-layering` when the ownership is ambiguous.
...
- **Advanced frontend work**: Custom JavaScript is allowed when the design needs behavior blocks cannot represent. Load `interactive-frontend` first; prefer custom blocks and the Interactivity API for reusable reactive behavior, and reserve plain enqueued JavaScript for progressive enhancement or intentionally frontend-only effects.

## Editor Parity

A visual design is not complete if the front end looks polished but the editor looks plain. Key design decisions that a non-developer should see or adjust — color, type scale, spacing, alignment, content width, repeated sections — should live in block attributes, `theme.json`, patterns, or templates. CSS may still create effects and unsupported polish, but it should not be the only place normal section styling exists.
