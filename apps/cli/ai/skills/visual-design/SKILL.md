...
Before you judge or fix layout (column counts, grids, spacing, alignment) from a screenshot, load the `verify-layout` skill — a screenshot is a measurement instrument with real limits, and that skill covers how to avoid chasing artifacts of downscaled captures.
