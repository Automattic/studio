...
## Contact sheet
The page is a photographer's contact sheet: a dark ground, a strict grid of uniform frames with a frame number under each, film-edge markings along the sides, and one frame circled in grease pencil as the featured shot. Clicking a frame opens it large. Text is confined to a typed label strip above the sheet and a short one below it.
Build: a `core/grid` or group with `grid-template-columns: repeat(auto-fill, minmax(220px, 1fr))` on a near-black group, each frame an image block with a thin light border, `aspect-ratio: 3 / 2`, and a small monospace caption for the number; film-edge strips as `repeating-linear-gradient` bands left and right; the circled frame via an absolutely positioned SVG ellipse in an accent color; a `<dialog>` with one image opened by a 15-line script.
Fallback: two columns on mobile; frames open in place without the dialog.
