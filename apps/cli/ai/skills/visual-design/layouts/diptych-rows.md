...
---
Build: alternate two-column groups with the grid layout in their markup (`"layout":{"type":"grid","columnCount":2}`, `gap: 0`, each column a cover or image block with `aspect-ratio: 4 / 5`) and single full-width cover or image blocks (`aspect-ratio: 21 / 9`), all `alignfull`; text rows as constrained groups with generous block padding; zero the theme block gap between rows (`margin-block-start: 0` on the row groups) so images touch; the header is a slim fixed bar with its own background.
