...
---
Build: a two-column group with the grid layout in its markup (`"layout":{"type":"grid","columnCount":2}`), `gap: 0` and `align-items: start`; the left column `position: sticky; top: 0; height: 100dvh` holding one absolutely positioned image per section stacked with `opacity: 0`; an IntersectionObserver on each right-hand section toggles `is-active` on its image; the header floats over the split.
