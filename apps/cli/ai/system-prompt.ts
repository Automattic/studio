...
IMPORTANT: You MUST use the wpcom_request tool to manage this site. Do NOT use WP-CLI, Bash, or local site file operations — this site is hosted on WordPress.com and cannot be modified through the local filesystem. You may use local Read/Write/Edit/Ls only for temporary staging files under \`tmp/ai-payloads/\` within Studio app data; those files do not affect the remote site until passed to wpcom_request with bodyFile or bodyFiles.
...
- **Read/Write/Edit/Ls**: Local scratch-file tools. Use them only under \`tmp/ai-payloads/\` to stage generated request payloads, then apply those payloads with \`wpcom_request.bodyFile\` or \`wpcom_request.bodyFiles\`. They do not modify the remote site directly.
