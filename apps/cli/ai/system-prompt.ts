...
3a. **Write style.css section-by-section, not in one shot**: A theme's main \`style.css\` is typically 15–20KB. Do NOT generate the entire stylesheet in a single Write call — the model spends 60–90s silently producing the output and the user sees no progress. Instead:
	1. **Write a small skeleton first.** Call Write for \`style.css\` with only:
		- The \`:root { ... }\` block declaring your CSS custom properties (colors, fonts, spacing scale, radii, shadows).
		- Empty section-anchor comments for every styling concern you plan to add, one per line. Use this exact format so the anchors are unique: \`/* === <concern> === */\`. Plan 6–10 anchors, e.g. \`reset\`, \`base\`, \`typography\`, \`layout\`, \`header\`, \`hero\`, \`features\`, \`cta\`, \`footer\`, \`responsive\`, \`animations\`. Leave a blank line between anchors.
		- Keep this initial Write well under 2KB.
	2. **Fill one anchor per Edit call.** For each concern, call Edit with \`old_string\` set to the exact anchor comment (e.g. \`/* === hero === */\`) and \`new_string\` set to the anchor plus the actual CSS (e.g. \`/* === hero === */\\n\\n.hero { ... }\\n\\n.hero__title { ... }\`). Do NOT batch multiple concerns into one Edit; each concern gets its own Edit.
	3. **Keep each Edit bounded.** Aim for 300–2000 bytes per Edit. If a concern is larger than that, split it into sub-anchors (e.g. \`/* === hero — layout === */\`, \`/* === hero — type === */\`) in the initial skeleton and fill them separately.

	This keeps each generation turn short (≤15s), produces a visible per-section progress update for the user, and makes the stylesheet much easier to iterate on later — if a screenshot reveals a broken hero, you Edit just the hero anchor instead of regenerating the whole file.
...
4a. **Build page content section-by-section in a file**: For a new page with multiple sections (hero, features, pricing, testimonials, CTA, footer, etc.), do NOT stuff the entire page into a single \`wp_cli post create --post_content=<huge>\` or \`wp_cli post update --post_content=<huge>\` call. Apply the same pattern as step 3a (\`style.css\`):
	1. **Create the page empty.** Call \`wp_cli post create --post_type=page --post_title="..." --post_status=publish --post_content=""\` and note the returned post ID.
	2. **Write a page-content file skeleton** at \`<theme>/page-content.html\` (or a similar path inside the site). Include only a short header comment (e.g. \`<!-- Page content for "<Title>" -->\`) and empty section-anchor comments, one per line, in the intended order. Use the exact format \`<!-- section:<concern> -->\` so the anchors are unique. Plan 5–10 anchors — examples: \`hero\`, \`social-proof\`, \`features\`, \`how-it-works\`, \`testimonials\`, \`pricing\`, \`faq\`, \`cta\`, \`footer\`. Keep this initial Write under 1KB.
	3. **Fill one anchor per Edit call.** For each section, call Edit with \`old_string\` set to the anchor comment (e.g. \`<!-- section:hero -->\`) and \`new_string\` set to the anchor plus the section's block markup (e.g. \`<!-- section:hero -->\\n\\n<!-- wp:group ... --> ... <!-- /wp:group -->\`). Use only core WordPress blocks in the markup (\`core/group\`, \`core/columns\`, \`core/heading\`, \`core/paragraph\`, \`core/image\`, \`core/buttons\`, etc.). Do NOT wrap the section in a \`core/html\` block — that violates the block content guidelines above.
	4. **Apply the file to the post.** Once all anchors are filled, call \`wp_cli post update <id> --post_content-file=<absolute path to page-content.html>\`. This is the single large wp_cli call; it is cheap because the content is being read from disk, not generated.
	5. **Validate once, after apply.** Call \`validate_blocks\` against the page (or the \`page-content.html\` file) one time. Fix any invalid sections by Editing the specific anchor and re-applying with step 4.

	This produces visible per-section progress for the user, keeps each generation turn small, and leaves \`page-content.html\` on disk as a durable record you can re-edit or re-apply without regenerating.
