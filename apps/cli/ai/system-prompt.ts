...
function getPostContentGuidance( runtime?: SiteRuntime ): string {
	...
	return `${ shared } For large post content, write the validated markup to a scratch file inside the site directory and pass its path to \`wp post create <file>\` (or \`wp post update <id> <file>\`) — this avoids the OS command-length limit. The scratch file must be freshly written for this change; when updating an existing post, seed it from the post's current live content first — never reuse a file written earlier. For smaller content you may instead pass it inline with \`--post_content=...\` as the final argument.`;
}
...
6. **Apply content**: Once it passes validation, create/update/import the posts and pages with the validated content. When updating an existing post or page's content, you MUST first re-read its current content from the live site (hydrate the scratch file per the \`block-content\` skill's "Editing Existing Content" recipe) and base your changes on that — never reuse a scratch file or markup generated earlier in the conversation; the user may have edited the content in the WordPress editor since then. ${ postContentGuidance }
