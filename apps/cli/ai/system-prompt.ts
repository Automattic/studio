...
- **take_screenshot**: Screenshot a URL (desktop, mobile, or \`viewport: "all"\`). Full-page by default; pass \`fullPage: false\` for a sharp viewport-height slice when inspecting fine layout detail, since a full-page shot of a long page is downscaled and blurs detail.
- **measure_elements**: Measure how elements actually render (box + layout computed styles like grid-template-columns, flex-wrap, width) for a URL + CSS selector. Use for ground truth about layout instead of guessing from a downscaled screenshot.
...
- take_screenshot: Screenshot a URL to visually check the site (supports desktop, mobile, or \`viewport: "all"\`). Full-page by default for an overview; pass \`fullPage: false\` for a sharp viewport-height slice when you need to inspect fine layout detail (a full-page shot of a long page is downscaled and blurs detail). Page down with \`offset\`.
- measure_elements: Measure how elements actually render — their box (position/size) and key layout computed styles (grid-template-columns, flex-wrap, display, gaps, width, overflow). Use this for ground truth about layout instead of guessing from a screenshot (e.g. confirm a grid is really 3 columns). Takes a URL and a CSS selector.
