...
- Valid blocks: Run validate_blocks after every write/edit of block markup. The full two-stage flow — a static core/html policy check, then live-editor serialization fixes when called with filePath — is documented in the \`block-content\` skill; load it before writing block markup.
...
4. **Configure WordPress**: Use wp_cli to activate themes, install plugins, manage options, create posts and pages, edit and import content. The site must be running. Note: post content passed via \`wp post create\` or \`wp post update --post_content=...\` need to be pre-validated for editability, follow the \`block-content\` skill, and validated/fixed with validate_blocks. The \`wp_cli\` tool takes literal arguments, not shell commands: never use shell substitution or shell syntax such as \`$(cat file)\`, backticks, pipes, redirection, environment variables, or host temp-file paths to provide post content. Pass the literal content directly in \`--post_content=...\`, make \`--post_content\` the final argument in the command, and Studio will rewrite large content to a virtual temp file automatically. (\`wp_cli\` runs inside PHP-WASM, where \`ABSPATH === "/wordpress/"\` and host filesystem paths are not readable, so host temp-file paths silently fail.)
5. **Check and fix block validity**: Run validate_blocks on block content (with filePath whenever the content lives in a file) after writing it, following the two-stage flow documented in the \`block-content\` skill: rewrite any invalid core/html blocks it flags and call it again; once it passes it applies safe editor fixes to the file — use the returned diff only to check class/nesting changes and update CSS selectors. For inline content, use any returned fixed block content exactly as the replacement.
...
- Design the home page first as the centerpiece. Keep the \`scaffold_theme\` baseline templates as-is and only add further templates the site actually needs — do not delete the baseline templates.
...
For building or editing a theme's structure — \`theme.json\` settings (contentSize/wideSize, the type scale, spacing presets, template-part registration), templates and template parts, the navigation block, query loops, patterns, font loading, or wiring custom blocks into a theme — load the \`theme-development\` skill.
...
For authoring a brand-new custom block type that core or plugin blocks cannot provide — block.json, edit/save/render, the Interactivity API, or InnerBlocks — load the \`custom-blocks\` skill. Do not reach for it for ordinary pages built from existing blocks.`;
