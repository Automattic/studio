...
5. **Check the misuse of HTML blocks**: Verify if HTML blocks were used as sections or not. If they were, use html_to_blocks to convert them to regular core blocks, then run block validation again.
...
- html_to_blocks: Get guidelines for converting raw HTML into WordPress block markup. Accepts HTML input (file or inline) and returns the conversion reference — block equivalents, attribute patterns, layout blocks. You do the conversion yourself. After converting, validate with validate_blocks and verify with take_screenshot.
