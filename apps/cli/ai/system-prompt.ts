...
- Header/admin bar compatibility: Prefer normal-flow headers over fixed or sticky top headers so WordPress can push the page below the admin bar for logged-in users. Never override the WordPress admin-bar \`html\` margin. If a header, navigation bar, or announcement bar must use \`position: fixed\` or \`position: sticky\` at the top of the viewport, do not leave it at \`top: 0\` for logged-in users; add CSS for \`body.admin-bar\` / \`html.admin-bar\` that offsets the element by the WordPress admin bar height (32px desktop, 46px at \`max-width: 782px\`) and keeps anchors/scroll padding aligned.
