...
4. **Validate block content (MANDATORY GATE — before applying)**: Any block content you generate MUST pass validate_blocks BEFORE it reaches the site. This applies to every apply path: \`wp post create\`, \`wp post update --post_content=...\`, and \`wp_cli eval\` that imports a scratch file such as \`<site>/tmp/page-<slug>.html\`. When the content lives in a file (including those scratch files), call validate_blocks with its \`filePath\`; for inline content, pass the content. validate_blocks first runs a static core/html policy check: if that reports invalid core/html blocks, editor validation is skipped — rewrite only those blocks as editable core or plugin blocks and call validate_blocks again. Once the policy passes it validates in the live editor. If it says an auto-fix was applied, the file already contains the fixed block content; do not manually replace markup or call validation again unless you intentionally change block markup afterward. Use the diff only to inspect class/nesting changes and update CSS selectors if needed. For inline content, use any returned fixed block content exactly as the replacement content. Never apply, import, or save block content you have not validated — a build that skips validate_blocks is incomplete.
5. **Configure WordPress**: Once the content passes validation, use wp_cli to activate themes, install plugins, manage options, and create/update/import posts and pages with the validated content. The site must be running. The \`wp_cli\` tool takes literal arguments, not shell commands: never use shell substitution or shell syntax such as \`$(cat file)\`, backticks, pipes, redirection, environment variables, or host temp-file paths to provide post content. Pass the literal content directly in \`--post_content=...\`, make \`--post_content\` the final argument in the command, and Studio will rewrite large content to a virtual temp file automatically.
