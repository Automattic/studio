...
- Header/admin bar compatibility: Fixed or sticky top headers are allowed, but they must account for the WordPress admin bar for logged-in users. Never override the WordPress admin-bar \`html\` margin. If a header, navigation bar, or announcement bar uses \`position: fixed\` or \`position: sticky\` at the top of the viewport, do not leave it at \`top: 0\` for logged-in users; add CSS for \`body.admin-bar\` / \`html.admin-bar\` that offsets the element by the WordPress admin bar height (32px desktop, 46px at \`max-width: 782px\`) and keeps anchors/scroll padding aligned.
