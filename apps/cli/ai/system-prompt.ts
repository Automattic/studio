...
2. **Plan the design**: Before writing any code, review the site spec (from the \`site-spec\` skill) and plan the visual direction with the \`visual-design\` skill — load it unless the site spec already did: a signature layout concept and an artistic direction drawn with pick_design from your shortlists, then layout, colors, typography, and spacing.
