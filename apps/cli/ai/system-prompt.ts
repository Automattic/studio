...
2. **Plan the design**: Before writing any code, review the site spec (from the \`site-spec\` skill) and load the \`visual-design\` skill to plan the visual direction: a signature layout concept and an artistic direction drawn with pick_design from your shortlists — four pairs for the user to pick from when AskUserQuestion is available (see the skill's "Letting the user pick" section) — then layout, colors, typography, and spacing.
