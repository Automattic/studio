...
3. **Write theme/plugin files**: For a brand new theme, call \`scaffold_theme\` first — it drops a minimal block-theme baseline (style.css, theme.json with appearanceTools and a sane palette/spacing scale, functions.php with frontend + editor style enqueue, default templates and parts, empty assets/fonts and patterns dirs) and activates it by default. Then use Write and Edit to fill the scaffold (one part/template/file per turn). Homepage layouts go in \`templates/front-page.html\`; reserve \`templates/page.html\` for interior pages. For plugins or for editing an existing theme, use Write and Edit directly under the site's wp-content/themes/ or wp-content/plugins/ directory.
...
- Homepage: \`Edit\` \`templates/front-page.html\` directly — replace the scaffolded \`<!-- wp:post-content /-->\` with \`<!-- section:<concern> -->\` anchors (one per planned section: hero, features, cta, etc.), then fill one anchor per Edit using only core blocks. WP renders this template at \`/\` automatically; no DB page or Reading-settings change needed.
