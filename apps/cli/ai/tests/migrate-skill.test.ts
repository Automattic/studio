...
/**
 * The `/migrate` wrapper skill is parsed by `loadSkills()` at runtime. The
...
describe( '/migrate skill', () => {
	const skillPath = path.resolve( import.meta.dirname, '..', 'skills', 'migrate', 'SKILL.md' );

	it( 'exists at apps/cli/ai/skills/migrate/SKILL.md', () => {
	...
	it( 'parses frontmatter with name=migrate and a non-empty description', () => {
		const skill = findSkill( 'migrate' );
	...
	it( 'points users to `studio migrate` for headless mode', () => {
		const skill = findSkill( 'migrate' );
		const body = skill!.body;
		expect( body.toLowerCase() ).toContain( 'studio migrate' );
	} );
} );
