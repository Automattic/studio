...
import { defineTool, type AnyStudioAgentTool } from './define-tool';
...
const backgroundGenerations = new Set< Promise< unknown > >();

export const waitForBackgroundImages = () => Promise.all( backgroundGenerations );

export function withBackgroundGeneration( tool: typeof generateImagesTool ): AnyStudioAgentTool {
	return {
		...tool,
		description: `${ tool.description } Pass \`background: true\` for the images of the design previews: the call returns at once, and present_design_options waits for the files.`,
		...
		execute: async (
			toolCallId,
			{ background, ...params }: Static< typeof tool.parameters > & { background?: boolean },
			signal,
			onUpdate
		) => {
			if ( ! background ) {
			...
			params.images.forEach( ( image ) => resolveImageFilePath( image.path ) );
			const generation: Promise< unknown > = tool
				.execute( toolCallId, params )
				.catch( () => undefined )
				.finally( () => backgroundGenerations.delete( generation ) );
			backgroundGenerations.add( generation );
			return textResult( 'Generating the images in the background.' );
		},
	};
}
