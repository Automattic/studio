...
// The tool is the same for text-only models; only how the prompt introduces it
// changes, since they cannot pair it with a capture.
export function createInspectDesignTool( { visionEnabled }: { visionEnabled: boolean } ) {
	return defineTool(
		'inspect_design',
...
export const inspectDesignTool = createInspectDesignTool( { visionEnabled: true } );
