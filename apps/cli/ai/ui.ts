...
export interface SiteInfo {
	...
	selfHostedUsername?: string;
	selfHostedAppPassword?: string;
}
...
export class AiChatUI implements AiOutputAdapter {
	...
	private async openSitePicker(): Promise< void > {
		...
		this.sitePickerSelfHostedItems = selfHostedSites.map( ( site ) => ( {
			...
			url: site.url,
			selfHostedSite: true,
			selfHostedUsername: site.username,
			selfHostedAppPassword: site.appPassword,
		} ) );
		...
		if ( answer === __( 'yes' ).toLowerCase() ) {
			try {
				...
				config.selfHostedSites = ( config.selfHostedSites ?? [] ).filter(
					( s ) => s.url !== site.url || s.username !== site.selfHostedUsername
				);
			...
			if ( this._activeSite?.selfHostedSite && this._activeSite?.url === site.url ) {
	...
	private async connectSelfHostedSite(): Promise< void > {
		...
		// Test connection by fetching site info from the REST API
		...
		const siteInfo: SiteInfo = {
			...
			url: normalizedUrl,
			selfHostedSite: true,
			selfHostedUsername: username,
			selfHostedAppPassword: appPassword,
		};
