...
`,
	...
	describe( 'getExtraDirectoryMounts', () => {
		it( 'rejects paths that escape the raw directory', () => {
		...
		it( 'mounts a reported extra directory from the raw tree', () => {
	...
	it( 'ensures sqlite integration is installed for imported sites', async () => {
		...
		const rawWpContent = path.join( importRoot, 'raw', 'srv', 'htdocs', 'wp-content' );
		...
		try {
			...
			await expect(
				ensureImportedSiteSqliteReady( runtimeBlueprintPath, '/srv/htdocs/wp-content' )
			...
			expect( installSqliteIntegrationMock ).toHaveBeenCalledWith(
				path.join( importRoot, 'raw', 'srv', 'htdocs' )
			);
		} finally {
			fs.rmSync( importRoot, { recursive: true, force: true } );
		}
