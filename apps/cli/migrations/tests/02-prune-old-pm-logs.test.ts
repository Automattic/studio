...
import { pruneOldPmLogs } from 'cli/migrations/02-prune-old-pm-logs';
...
describe( 'pruneOldPmLogs', () => {
	describe( 'needsToRun', () => {
		it( 'returns false when the logs directory does not exist', async () => {
			fs.rmSync( logsDir, { recursive: true, force: true } );
			expect( await pruneOldPmLogs.needsToRun() ).toBe( false );
		} );

		it( 'returns false when the logs directory is empty', async () => {
			expect( await pruneOldPmLogs.needsToRun() ).toBe( false );
		} );

		it( 'returns false when all logs have fresh mtimes', async () => {
			writeFile( `site-out-${ dateTag( daysAgo( 30 ) ) }.log`, daysAgo( 1 ) );
			writeFile( 'site-error.log', daysAgo( 2 ) );
			expect( await pruneOldPmLogs.needsToRun() ).toBe( false );
		} );

		it( 'returns true when a dated log has a stale mtime', async () => {
			writeFile( `site-out-${ dateTag( daysAgo( 1 ) ) }.log`, daysAgo( 20 ) );
			expect( await pruneOldPmLogs.needsToRun() ).toBe( true );
		} );

		it( 'returns true when a legacy log has a stale mtime', async () => {
			writeFile( 'site-out.log', daysAgo( 20 ) );
			expect( await pruneOldPmLogs.needsToRun() ).toBe( true );
		} );
	} );
	...
	describe( 'run', () => {
		it( 'deletes dated logs whose mtime is past the retention window', async () => {
			...
			const fresh = writeFile(
				`site-out-${ dateTag( daysAgo( 1 ) ) }.log`.replace( 'site-out', 'site-other-out' ),
				daysAgo( 1 )
			);

			await pruneOldPmLogs.run();

			expect( fs.existsSync( stale ) ).toBe( false );
			expect( fs.existsSync( fresh ) ).toBe( true );
		} );

		it( 'deletes legacy logs whose mtime is past the retention window', async () => {
		...
		it( 'preserves actively-written files regardless of the filename date tag', async () => {
			...
			const active = writeFile(
				`long-running-out-${ dateTag( daysAgo( 60 ) ) }.log`,
				daysAgo( 0 )
			);
			await pruneOldPmLogs.run();
		...
		it( 'is a no-op when the logs directory does not exist', async () => {
			fs.rmSync( logsDir, { recursive: true, force: true } );
			await expect( pruneOldPmLogs.run() ).resolves.toBeUndefined();
		} );
	} );
} );
