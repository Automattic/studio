...
describe( 'runPollLoop', () => {
	...
	describe( 'Dolly bump stats', () => {
		...
		it( 'records an auth-error detach when polling returns a RemoteAuthError', async () => {
			const deps = makeDeps( {} );
			const { RemoteAuthError } = await import( 'cli/remote-session/remote-http' );
			( deps.poll as ReturnType< typeof vi.fn > ).mockRejectedValueOnce(
				new RemoteAuthError( 401 )
			);
