...
export function Toolbar( { start, middle, end, className, startInset }: ToolbarProps ) {
	return (
		<div className={ cx( 'relative flex items-center p-2 flex-shrink-0 min-h-[40px]', className ) }>
			{ /* Start — left-aligned */ }
			<div
				className="flex items-center gap-1 z-10 min-w-0 flex-1 transition-[padding] duration-200 ease-in-out"
			...
			{ /* End — right-aligned */ }
			<div className="flex items-center gap-1 flex-shrink-0 z-10">{ end }</div>
		</div>
	);
}
