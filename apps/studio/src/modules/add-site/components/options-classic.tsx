...
export type ClassicAddSiteFlowType =
	| 'create'
	| 'blueprint'
	| 'blueprintDeeplink'
	| 'backup'
...
interface AddSiteOptionsProps {
	onOptionSelect: ( option: ClassicAddSiteFlowType ) => void;
}
	...
	return (
		...
		>
			<HStack
				as="button"
				className={ cx(
					'w-full p-4 border border-frame-border rounded-xl text-left',
					'rtl:text-right',
...
export default function AddSiteOptionsClassic( { onOptionSelect }: AddSiteOptionsProps ) {
	const { __ } = useI18n();
	...
	return (
		<VStack className="text-center w-full" alignment="top" spacing="3">
			...
			<Text className="text-[15px] font-light text-frame-text-secondary max-w-sm mb-4">
