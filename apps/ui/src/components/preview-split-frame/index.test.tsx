...
import { PreviewSplitFrame, type PreviewSplitFramePreviewProps } from './index';
...
describe( 'PreviewSplitFrame', () => {
	...
	it( 'lays out the preview immediately when mounted open', async () => {
		const preview = vi.fn( ( { collapsed }: PreviewSplitFramePreviewProps ) => (
			<aside aria-label="Site preview" data-collapsed={ collapsed ? 'true' : 'false' } />
		) );
		...
		await waitFor( () => {
			...
			expect( root ).toHaveStyle( '--preview-frame-preview-width: 400px' );
		} );
