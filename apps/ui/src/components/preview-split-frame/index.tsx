...
export function PreviewSplitFrame( {
	...
	const measureRootWidth = useCallback( () => {
		...
		if ( rootWidthRef.current !== roundedWidth ) {
	...
	const applyContentWidth = useCallback(
		( nextContentWidth: number, containerWidth = rootWidthRef.current ) => {
			...
			setContentWidth( ( current ) =>
				current === clampedContentWidth ? current : clampedContentWidth
			);
	...
	useLayoutEffect( () => {
		...
		const updateRootWidth = ( measuredWidth?: number ) => {
			const nextRootWidth = Math.round( measuredWidth ?? root.getBoundingClientRect().width );
			if ( nextRootWidth <= 0 ) {
				return;
			}
			const rootWidthChanged = rootWidthRef.current !== nextRootWidth;
			if ( ! rootWidthChanged && contentWidthRef.current !== null ) {
				return;
			}
			...
			if ( previewOpenRef.current && ! previewTransitioningRef.current ) {
				...
				setContentWidth( ( current ) => ( current === nextRootWidth ? current : nextRootWidth ) );
			}
			if ( rootWidthChanged ) {
		...
		const scheduleRootWidthUpdate = ( measuredWidth?: number ) => {
			pendingRootWidth = measuredWidth;
			if ( resizeFrame !== undefined ) {
				return;
			}
			resizeFrame = window.requestAnimationFrame( () => {
				resizeFrame = undefined;
				const nextRootWidth = pendingRootWidth;
				pendingRootWidth = undefined;
				updateRootWidth( nextRootWidth );
			} );
		};
		if ( typeof ResizeObserver === 'undefined' ) {
			const handleWindowResize = () => scheduleRootWidthUpdate();
		...
		const resizeObserver = new ResizeObserver( ( entries ) => {
			scheduleRootWidthUpdate( entries[ 0 ]?.contentRect.width );
		} );
	...
	return (
		...
		>
			<div
				className={ clsx( styles.contentColumn, contentMode === 'raw' && styles.contentColumnRaw ) }
			>
				{ contentMode === 'raw' ? (
					children
				) : (
		...
		<div ref={ scrollRef } className={ clsx( styles.scroll, scrollClassName ) }>
			...
			{ composer !== undefined ? (
				<div className={ clsx( styles.composerOuter, composerOuterClassName ) }>
					...
					<div className={ styles.composerContent }>{ composer }</div>
				</div>
			) : null }
		</div>
	);
