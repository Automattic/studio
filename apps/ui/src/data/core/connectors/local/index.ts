...
export function createLocalConnector( { apiBaseUrl }: LocalConnectorOptions ): Connector {
	...
	return {
		...
		async readBlueprintFile(): Promise< Record< string, unknown > > {
			// Only reachable from the desktop's add-site-with-blueprint menu
			// event, which never fires in a browser (see below).
			throw new UnsupportedError( 'readBlueprintFile' );
		},
		onAddSiteRequested() {
			// Desktop application-menu events — no menu in a browser tab.
			return () => {};
		},
		...
		async importSiteFromBackup( siteId, backup ): Promise< void > {
			await api( `/sites/${ encodeURIComponent( siteId ) }/import`, {
				method: 'POST',
