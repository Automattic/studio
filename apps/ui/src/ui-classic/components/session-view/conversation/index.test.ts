...
describe( 'Assistant message copy button', () => {
	it( 'copies the full message when text blocks are split by tool calls', () => {
		...
		expect(
			screen.getByText( 'First part.' ).closest( '[data-message-text]' )
		).toHaveAttribute( 'data-message-text', 'First part.\n\nSecond part.' );
		expect(
			screen.getByText( 'Second part.' ).closest( '[data-message-text]' )
		).toHaveAttribute( 'data-message-text', 'First part.\n\nSecond part.' );
