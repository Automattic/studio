...
import { SIMULATED_WPORG_USERNAME, useWporgConnected } from '@/lib/wporg-connection';
...
// Signed-out state — a benefits list plus a "log in" button that opens the
// simulated-login dialog (WordPress.org login can't be automated: reCAPTCHA
// + mandatory 2FA). Mirrors the site connect screen's WordPress.com block.
	...
	return (
		<div className={ styles.signedOut }>
			...
			<div className={ styles.authActions }>
				<Button type="button" variant="solid" tone="brand" onClick={ onLogin }>
					<Icon icon={ wordpress } />
...
function PluginConnectPage() {
	...
	const {
		...
		isLoading,
		isError,
	} = useWporgAuthorPlugins( connected ? SIMULATED_WPORG_USERNAME : undefined );
	...
	return (
		<div className={ `${ sharedStyles.page } ${ sharedStyles.pageSpacious }` }>
			...
			{ ! connected && <SignedOutView onLogin={ () => setLoginDialogOpen( true ) } /> }
