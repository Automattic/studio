...
describe( 'DeskMenu', () => {
	...
	it( 'does not render UI mode switchers in the desk menu', async () => {
		render( <DeskMenu /> );

		fireEvent.click( screen.getByRole( 'button', { name: 'Desk menu' } ) );

		expect( await screen.findByRole( 'menuitem', { name: 'User desk' } ) ).toBeInTheDocument();
		expect(
			screen.queryByRole( 'menuitem', { name: 'Switch to old Studio UI' } )
