...
const WIDGET_VIGNETTE_MAX_SIZE = 144;
const WIDGET_VIGNETTE_FALLBACK_SIZE = 200;
...
function getWidgetSummary( widget: DeskWidget ) {
	switch ( widget.type ) {
		...
		case 'media':
			return (
				truncateText( stripMarkup( widget.widgetProps.alt ), 72 ) ||
				truncateText( widget.widgetProps.url, 72 ) ||
				__( 'Media item' )
			);
		case 'post':
			return sprintf(
...
function getWidgetPromptContext( widget: DeskWidget ) {
	switch ( widget.type ) {
		...
		case 'media':
			return formatPromptContext( widget, {
				alt: stripMarkup( widget.widgetProps.alt ),
				mediaId: widget.widgetProps.mediaId,
				mediaKind: widget.widgetProps.mediaKind,
