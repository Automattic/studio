...
| `studio_setting_ai_provider_change` | `saveAnthropicApiKey` (`surface=settings`), `setAiSessionProvider` (`surface=conversation`) | `provider` (`wpcom`/`anthropic-api-key`), `surface` — the API key is **never** sent. `settings`: the saved key changed (`has_anthropic_api_key` boolean; clearing the key also falls the default provider back to WordPress.com). `conversation`: the composer pinned a conversation to a provider (`model` rides along; session ids are never sent). Browser-hosted `studio ui` goes through `apps/local`, which has no Tracks emitter, so its changes go uncounted — see [STU-2247](https://linear.app/a8c/issue/STU-2247). |
