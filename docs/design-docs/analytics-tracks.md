...
| Event | Emitted from | Event-specific props |
|---|---|---|
| `studio_code_message_sent` | CLI `runAgentTurn` | `provider` (`wpcom`/`anthropic-api-key`/`openai-compatible` — the gateway serving the request, not the model vendor), `model` (e.g. `claude-sonnet-5`; a model outside the built-in catalog, i.e. one named by a local `openai-compatible` endpoint, is reported as `local` — the id can be a filesystem path), `model_family` (`anthropic`/`studio`/`openai`), `ability_name` (predefined skill, absent for an ordinary message), `has_images`, `has_files` (booleans). One per user turn dispatched. |
