...
| `studio_setting_ai_provider_change` | `saveAnthropicApiKey` | `provider` (`wpcom`/`anthropic-api-key`), `has_anthropic_api_key` (boolean), `surface` (`settings`) — the API key is **never** sent. Fires when the saved key changes; clearing the key also falls the default provider back to WordPress.com. Browser-hosted `studio ui` writes these settings through `apps/local`, which has no Tracks emitter, so those changes go uncounted — see [STU-2247](https://linear.app/a8c/issue/STU-2247). |
