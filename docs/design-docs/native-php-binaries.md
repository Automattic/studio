...
At runtime, Studio checks the Apps CDN manifest for the requested PHP minor
version and downloads the latest matching patch for the current platform and
architecture. Otherwise the native PHP install fails; Apps CDN is the source of
truth for these binaries.
