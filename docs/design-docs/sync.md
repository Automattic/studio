...
- **Sync connection**: A Sync connection is a relationship between a local machine and a remote site. That information lives in `shared.json` under `connectedWpcomSites`, keyed by user id, and is read/written through `tools/common/lib/connected-sites.ts` so both Studio and the Studio CLI can update it.
