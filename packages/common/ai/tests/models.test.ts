...
describe( 'resolveSessionModel', () => {
	...
	it( 'returns the fallback for a session with no recorded model', () => {
		expect( resolveSessionModel( [], 'claude-opus-5' ) ).toBe( 'claude-opus-5' );
	} );

	it( 'keeps the recorded model over the fallback', () => {
		expect( resolveSessionModel( [ modelChangeEntry( 'gpt-5.6-sol' ) ], 'claude-opus-5' ) ).toBe(
			'gpt-5.6-sol'
		);
	} );
	...
	it( 'falls back when the recorded model is no longer offered', () => {
		expect(
			resolveSessionModel( [ modelChangeEntry( 'claude-retired-1' ) ], 'claude-opus-5' )
		).toBe( 'claude-opus-5' );
	} );
} );

describe( 'getAiModelThinking', () => {
	// Sonnet 5 and Opus 5 only accept `thinking: {type: "adaptive"}` —
	...
	it( 'marks current Anthropic models as adaptive-thinking', () => {
		...
		expect( getAiModelThinking( 'claude-opus-5' ) ).toBe( 'adaptive' );
	} );
