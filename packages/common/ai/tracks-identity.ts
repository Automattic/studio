...
// Identity props shared by every Studio Code event, so the CLI and Main emitters can't drift. No
// `agent_version`: pi is pinned per release, so `app_version` determines it.
