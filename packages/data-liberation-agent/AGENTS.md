...
- Two branches that both regenerate the bundles conflict on them by construction — they're minified, so any two rebuilds differ byte-for-byte even when the underlying source changes don't overlap. `.gitattributes` declares `merge=ours` for both bundle files so git takes one side instead of attempting a textual merge. **Never resolve a bundle conflict by hand-merging the hunks** — a textual merge on minified output can merge cleanly and still be wrong. Regenerate with `npm run build:mcp-bundle` and commit the result; "Verify committed plugin bundles" confirms the committed bytes match a fresh build. GitHub's own conflict check does not run git merge drivers, so its PR banner can still show these files as conflicting even with the attribute in place — pull the branch locally and merge or rebase there, where the attribute takes effect, then push.
