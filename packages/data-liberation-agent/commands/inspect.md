...
- **Platform features** — detects stores, bookings, forms, members areas, scheduling, forums, events, restaurants (menus/ordering/reservations), pricing plans, multilingual sites, and CMS/dynamic pages or custom code. Detection uses sitemap URL patterns plus markup markers from a sample of fetched pages (homepage first). Each feature reports which signal fired (`evidence: url | html`), whether it transfers automatically, and a WordPress plugin recommendation if it doesn't.
