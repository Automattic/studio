...
The recommendation follows a fixed rule computed from the discovery inventory: a store or any products → **blocks + products**; otherwise more posts than pages (content-dominant) → **blocks + products**; otherwise (page-dominant, fixed-layout marketing site) → **theme replication**. The same inventory always yields the same recommendation. The choice costs nothing to change — capture is reusable, so re-running `/liberate` on an already-captured site jumps straight back to this question and you can try the other path without re-capturing.
