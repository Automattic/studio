...
- **`skills/replicate-with-blocks/styling-priority.md`** — the preset→patch→instance→variation→layout→CSS cascade, the structured-props cheat sheet, and the hard bans (no raw style="" attrs, no invented classNames in place of the cascade — scoped-parity wrapper classNames per channel 7c are the sanctioned exception). Applies to native block output; core/html islands exempt.
