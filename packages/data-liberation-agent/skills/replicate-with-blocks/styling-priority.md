...
- **Invented classNames as a substitute for the structured cascade (options 1–6).** If you want `&.my-made-up-class{...}` for something the cascade can express, you actually want: a variation (`is-style-lib-<slug>` — registered, not invented), structured properties, or a `layout` attribute.
  Exception — the parity workflow (channel 7c): when the cascade genuinely cannot express a source treatment, a stable wrapper className (e.g. `specialty-grid`, `pill-row`, `is-replica-card`) whose only job is to scope a `style.css` rule IS legal — this is the "Reach parity with core blocks + scoped theme CSS" lever in the replicate-with-blocks SKILL, and the deterministic pipeline emits such hooks itself (e.g. `site-header-overlay`). Exhaust options 1–6 first; the className must not stand in for a structured attribute that exists.
  Otherwise-legal classNames: `is-style-<registered-slug>`, wp-core classes (`alignwide`, `alignfull`), classes the source markup requires for behavior.
