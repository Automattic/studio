...
export async function discover(
	...
	try {
		...
		async function fetchSitemapPw(
			...
			const fail = ( reason: string ): void => {
				if ( speculative ) {
					if ( wixOpts.verbose ) {
						console.warn(
							`[wix:discover] optional sub-sitemap not present: ${ sitemapUrl } (${ reason })`
						);
					}
					return;
				}
				console.warn(
					`[wix:discover] sitemap fetch failed after ${ RETRIES } attempts: ${ sitemapUrl } (${ reason })`
				);
		...
		if ( sitemapFailures.length > 0 ) {
			console.warn(
				`[wix:discover] ${ sitemapFailures.length } sitemap fetch(es) failed — inventory may be incomplete`
			);
		}
