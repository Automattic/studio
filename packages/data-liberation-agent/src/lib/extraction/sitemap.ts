...
function decodeXml( value: string ): string {
	return value.replace( /&(?:amp|lt|gt|quot|apos);|&#(?:x[\da-f]+|\d+);/gi, ( entity ) => {
		...
		const codePoint = Number(
			numeric.startsWith( 'x' ) || numeric.startsWith( 'X' ) ? `0${ numeric }` : numeric
		);
...
export function parseSitemapDocument( xml: string ): SitemapDocument {
	const kind = /<\s*(?:\w+:)?sitemapindex\b/i.test( xml )
		? 'index'
...
export function classifyUrl( url: string ): UrlType {
	...
	if ( /\/(?:category|categories|collections|product-category|product-tag)(?:\/|$)/.test( path ) )
		return 'page';
