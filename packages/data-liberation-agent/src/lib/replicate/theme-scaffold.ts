...
interface DesignFoundation {
	color?: {
		surface?: {
			base?: FoundationRoleValue;
			raised?: FoundationRoleValue;
			inverse?: FoundationRoleValue;
		};
		text?: {
			default?: FoundationRoleValue;
			muted?: FoundationRoleValue;
			subtle?: FoundationRoleValue;
			inverse?: FoundationRoleValue;
		};
		accent?: {
...
export function buildThemeScaffold( opts: ThemeScaffoldOpts ): ReplicaFile[] {
	...
	const themeDescription =
		opts.themeDescription ??
		`Replica theme generated by data-liberation. Tokens derived from ${
			opts.siteTitle ?? 'the source site'
		}.`;
	...
	const headerHtml = buildHeaderPart( {
		...
		localLogoUrl: opts.localLogoPath
			? `/wp-content/themes/${ themeSlug }/${ opts.localLogoPath.replace( /^\/+/, '' ) }`
			: undefined,
	} );
	...
	return [
		{
			...
			content: buildStyleCss( { themeName, themeSlug, themeDescription, capturedFonts } ),
		},
		{
			relativePath: 'theme.json',
			content: buildThemeJson( foundation, {
				capturedFonts,
				headingLineHeights: opts.headingLineHeights,
...
function buildReconstructedPageTemplates( pages: ReconstructedPage[] ): ReplicaFile[] {
	...
	for ( const page of pages ) {
		...
		if ( parts.length !== 2 || ! isSafeWpSlug( parts[ 0 ] ) || ! isSafeWpSlug( parts[ 1 ] ) )
			continue;
...
function buildStyleCss( args: {
	...
	return (
		buildThemeHeader( [
			...
			[
				'Theme URI',
				'https://github.com/Automattic/studio/tree/trunk/packages/data-liberation-agent',
			],
...
interface BlockOverride {
	...
	spacing?: { padding?: { top?: string; right?: string; bottom?: string; left?: string } };
...
function buildHeaderPart(
	args: {
		themeSlug?: string;
		chrome?: NonNullable< ThemeChromeEvidence[ 'header' ] >;
		localLogoUrl?: string;
	} = {}
): string {
	...
	const brand = logoUrl
		? `<!-- wp:image ${ jsonAttr( {
				url: logoUrl,
				alt: args.chrome?.logoAlt ?? 'Site logo',
				width: 150,
				linkDestination: 'custom',
				href: '/',
		  } ) } -->
<figure class="wp-block-image is-resized"><a href="/"><img src="${ escapeAttr(
				logoUrl
		  ) }" alt="${ escapeAttr(
				args.chrome?.logoAlt ?? 'Site logo'
 ...
 */
function buildHeaderIcons( args: {
	themeSlug?: string;
	...
	const asset = ( name: string ): string =>
		`/wp-content/themes/${ slug }/assets/icon-${ name }.svg`;
...
<figure class="wp-block-image size-full is-resized clone-header-icon"><a href="${ href }"><img src="${ asset(
			name
		) }" alt="${ label }" style="width:22px;height:22px"/></a></figure>
<!-- /wp:image -->`;
...
function buildFooterPart( args: {
	...
	const contactLinks = links.filter(
		( l ) => l.href.startsWith( 'tel:' ) || l.href.startsWith( 'mailto:' )
	);
	...
	const creditLinks = links.filter(
		( l ) => l.external && l.label && copyrightJoined.includes( l.label.toLowerCase() )
	);
	const navLinks = links
		.filter( ( l ) => ! contactLinks.includes( l ) && ! creditLinks.includes( l ) )
		.slice( 0, 10 );
...
<!-- /wp:paragraph -->` );
	...
	if ( creditLinks.length ) {
		...
		copyrightHtml = copyrightHtml.includes( escLabel )
			? copyrightHtml.replace( escLabel, anchor )
			: `${ copyrightHtml } ${ anchor }`.trim();
	}
...
function remapLinks(
	...
	remap: NavHrefRemap
): { links: ThemeChromeLink[]; dropped: number } {
