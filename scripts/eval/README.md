...
- **skill-routing-hardened** — A casual, speed-pressured design ask ("real quick, nothing fancy — drop a hero on the homepage") must still load `block-content` before writing block markup and load `visual-design` at all. KNOWN GAP as of 2026-07: fails on trunk and with strengthened Skill-tool rules alike (`visual-design` skipped 6/6 runs) — kept as the falsification target for whatever fix closes it.
