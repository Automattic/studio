...
- **active-site-reused** — Site pick: seeds a real site as the active site and asks for a build that names no other site. Asserts the agent works on the active site (`site_info` / `site-spec` skill) and never calls `site_create`.
- **active-site-different-name** — Site pick: same seed, but the prompt names a different site. Asserts the agent asks whether to reuse the active site or create a new one, and does not call `site_create` before the answer.
...
- **single-page-build-turn-cadence** — Agent builds a simple one-page site. Asserts (a) the default flow scaffolds a fresh blank theme (`scaffold_theme` without `parentTheme`, successfully), (b) every individual turn stays under 60s (wall-clock between successive assistant messages) (c) no `wp_cli` call uses `--post_content-file=` (which silently fails inside PHP-WASM) and (d) the agent loads the `visual-design` skill, calls `pick_design` for the look and the layout, states `Concept:` and `Direction:` lines in the Site Spec naming the layout concept and artistic direction the tool drew (or returned for a brief that named them), with a `Layout map` covering the page's sections, and writes the look to `DESIGN.md` at the site root with its tokens in the front matter.
- **one-page-nav-anchor-links** — Agent builds a one-page site whose header navigation jumps to page sections. Asserts the build produces `wp:navigation-link` blocks with anchor URLs and that none use bare-hash URLs (`#contact`), which resolve against the current URL and break on every page except home — anchor nav links must be home-relative (`/#contact`).
...
- **design-system-annotation** — An annotation from the preview's Design system page (the message the app sends, naming the `colors.primary` token) on a site seeded with a DESIGN.md. Asserts the agent changes the primary color in DESIGN.md and updates a theme.json (or scaffolds a theme from DESIGN.md).
...
Set a `seed` var on a test to pre-populate config files before the agent turn — useful for flows that depend on connected remote sites or preview sites without making a real WordPress.com connection or preview. The seed accepts `localSite` (written to `cli.json`), `activeSite` (creates a real stopped site under `~/Studio/`, with optional `files` written into it by path relative to the site root, and prefixes the prompt with the `[Active site: …]` line the app and CLI picker add; the site is deleted after the turn), `connectedWpcomSites` (written to `shared.json`, requires `studio auth login`), `snapshots` (written to `cli.json`), and `globalInstructions` (written to `~/.studio/knowledge/instructions.md`; the file's prior content — or absence — is restored after the turn). Everything seeded is removed automatically after the turn so reruns start clean.
