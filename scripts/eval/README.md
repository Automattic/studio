...
- **stale-content-hydration** — Regression for STU-2048. Creates a page, simulates an out-of-band WordPress-editor edit (image URL swap via a direct full-content update), then requests a heading-only change. Asserts the agent re-reads the live `post_content` between the out-of-band edit and its own apply (the block-content skill's hydration recipe, or a read inside the apply command), and that the final page content keeps the swapped image with the new heading (no reversion to the stale placeholder).
