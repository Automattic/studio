...
export function EmptyLayout( { footer, children }: EmptyLayoutProps ) {
	return (
		<div className="w-full h-full rounded-sm border border-a8c-gray-5 flex-col justify-between items-center inline-flex">
			<div className="self-stretch grow flex-col justify-center items-center flex">
				{ children }
			</div>
			{ footer && (
				<div className="flex-col justify-center items-center gap-2 flex">
					<div className="text-center text-a8c-gray-70 text-xs mb-6">{ footer }</div>
				</div>
			) }
		</div>
	);
...
EmptyLayout.Description = ( { children }: PropsWithChildren ) => (
	<div className="pt-2 w-72 text-center text-a8c-gray-70 text-body">{ children }</div>
);
