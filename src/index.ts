	...
	app.on( 'ready', async () => {
		...
		session.defaultSession.webRequest.onHeadersReceived( ( details, callback ) => {
			...
			const basePolicies = [
				...
				"script-src 'self' 'wasm-unsafe-eval' data:*", // allow WebAssembly to compile and instantiate
			];
			const prodPolicies = [ "connect-src 'self' data: https://public-api.wordpress.com" ];
			const devPolicies = [
				...
				"connect-src 'self' https://public-api.wordpress.com data: ws://localhost:*",
			];
			const policies = [
				...basePolicies,
