...
describe( 'setupAdminer', () => {
	...
	it( 'should copy adminer setup files to site path', async () => {
		...
		expect( recursiveCopyDirectory ).toHaveBeenCalledWith(
			'vendor/adminer',
			'mock-site-path/adminer'
		);
		expect( fs.promises.copyFile ).toHaveBeenCalledWith(
			'vendor/adminer/config.php',
			'mock-site-path/adminer/config.php'
		);
	...
	it( 'should not update adminer when versions match', async () => {
		...
		// Verify that only version checks were performed
		expect( fs.promises.readFile ).toHaveBeenCalledWith( 'vendor/adminer/version.txt', 'utf8' );
		expect( fs.promises.readFile ).toHaveBeenCalledWith(
			'mock-site-path/adminer/version.txt',
			'utf8'
		);
	} );
} );
...
describe( 'deleteAdminer', () => {
	it( 'should move adminer directory to trash', async () => {
		...
		expect( shell.trashItem ).toHaveBeenCalledWith( 'mock-site-path/adminer' );
	} );
