...
export async function installInstructionFile(
	...
	if ( ! overwrite ) {
		try {
			...
			// File does not exist, continue with install.
		}
	} else {
		overwritten = true;
	}
...
export function getAgentInstructionsPath( sitePath: string ): string {
	return getInstructionFilePath( sitePath, 'agents' );
}

export async function getAgentInstructionsStatus(
	sitePath: string
): Promise< { exists: boolean; path: string } > {
	const status = await getInstructionFileStatus( sitePath, 'agents' );
	return { exists: status.exists, path: status.path };
}
...
export async function installAgentInstructions(
	...
	return installInstructionFile( sitePath, 'agents', content, overwrite );
}
