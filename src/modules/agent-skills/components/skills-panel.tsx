/**
 * SkillsPanel - Main panel for managing site skills.
 *
 * Displays installed and available skills in a single view.
 */
...
export function SkillsPanel( { siteId, className }: SkillsPanelProps ) {
	...
	useEffect( () => {
		if ( availableSkills.length === 0 && ! isLoadingAvailable && ! availableError ) {
			void refreshAvailable();
		}
	}, [ availableSkills.length, isLoadingAvailable, availableError, refreshAvailable ] );
	...
	// Filter available skills that aren't installed
				...
				<div className="px-3 py-2 text-[11px] font-medium text-gray-600 uppercase tracking-wide bg-gray-50 border-b border-gray-200 border-t flex items-center justify-between">
					...
					{ uninstalledSkills.length > 0 && (
						<Button
							...
							disabled={ isInstalling }
							className="text-[11px] !py-0 !px-0 normal-case !font-medium hover:!text-a8c-blue-50"
						>
