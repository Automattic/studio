...
export const useSyncDialogTexts = ( type: 'pull' | 'push' ) => {
	...
	return useMemo( () => {
		const getEnvironmentCopy = ( envType: string ) => {
			const envLabel = getEnvironmentLabel( envType );

			if ( type === 'pull' ) {
				return {
					/* translators: %s is the environment name (e.g., "Production", "Staging") */
					title: sprintf( __( 'Pull from %s' ), envLabel ),
					/* translators: %s is the environment type (e.g., "production", "staging") */
					description: sprintf(
						__(
				...
				return {
					...
					/* translators: first %s is the environment type, second %s is the environment type (e.g., "production", "staging") */
					description: sprintf(
						__(
