...
export function EditVipEnvironment( { environment }: EditVipEnvironmentProps ) {
	...
	const handleSubmit = async ( event: FormEvent ) => {
		...
		try {
			...
			if ( phpVersion !== environment.phpVersion ) {
			...
			if ( xdebug !== environment.xdebug ) {
				args.push( `--xdebug=${ xdebug ? 'y' : 'n' }` );
			}

			if ( mailpit !== environment.mailpit ) {
				args.push( `--mailpit=${ mailpit ? 'y' : 'n' }` );
			}

			args.push( '--yes' );

			const result = await getIpcApi().executeVipCliCommand( args );
