...
export function VipContentTabImportExport( {
	...
	const handleImportSql = async () => {
		...
		try {
			...
			// Execute vip dev-env import sql command
			const importResult = await getIpcApi().executeVipCliCommand( [
				'dev-env',
				'import',
				'sql',
				result.path,
			...
			if ( importResult.success ) {
				setImportResult( {
					success: true,
					message: __( 'SQL file imported successfully.' ),
				} );
			} else {
				setImportResult( {
					success: false,
	...
	const handleImportMedia = async () => {
		...
		try {
			...
			// Open folder dialog to select media directory
			const result = await getIpcApi().showOpenFolderDialog(
			...
			// Execute vip dev-env import media command
			const importResult = await getIpcApi().executeVipCliCommand( [
				'dev-env',
				'import',
				'media',
			...
			if ( importResult.success ) {
				setImportResult( {
					success: true,
					message: __( 'Media files imported successfully.' ),
				} );
			} else {
				setImportResult( {
					success: false,
