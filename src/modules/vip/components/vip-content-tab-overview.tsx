...
function CustomizeSection( {
	...
	// VIP sites typically use block themes, so we provide block theme buttons by default
...
function ShortcutsSection( {
	...
	const path = selectedEnvironment.appCodePath || selectedEnvironment.path;
	...
	if ( editor && editorConfig ) {
		buttonsArray.push( {
			...
			onClick: async () => {
				if ( path ) {
					await getIpcApi().openAppAtPath( editor, path );
				}
			},
			disabled: ! path,
		} );
	...
	buttonsArray.push( {
		...
		onClick: async () => {
			if ( path ) {
				try {
					await getIpcApi().openTerminalAtPath( path );
				} catch ( error ) {
	...
	const services = [
		{
			...
			enabled: selectedEnvironment.elasticsearch,
			url: null,
		},
		{
			name: __( 'phpMyAdmin' ),
			enabled: selectedEnvironment.phpmyadmin,
			url: phpMyAdminUrl,
		},
		{
			name: __( 'Xdebug' ),
			enabled: selectedEnvironment.xdebug,
			url: null,
		},
		{
			name: __( 'Mailpit' ),
			enabled: selectedEnvironment.mailpit,
			url: mailpitUrl,
		},
	];
