...
import { VipBadge } from 'src/modules/add-site/components/vip-icon';
					...
					>
						...
						<div
							className={ cx(
								...
								classCircle,
								environment.running && 'border-[#00BA37] bg-[#1ED15A] duration-100',
								! environment.running && 'border-[#ffffff19] bg-[#ffffff26]'
							) }
						>
							&nbsp;
						</div>
...
export default function VipSiteMenu() {
	const {
		environments,
		selectedEnvironment,
		isLoading,
	...
	const handleSelectEnvironment = ( slug: string ) => {
		selectEnvironment( slug );
	};

	// Don't show anything if VIP CLI is not available
	if ( ! isVipAvailable && ! isLoading ) {
		return null;
	}

	// Don't show anything if no VIP environments exist
				...
				<ul className="pt-px">
					{ environments.map( ( env ) => (
						<VipSiteItem
							...
							onSelect={ () => handleSelectEnvironment( env.slug ) }
